...
class DataGenerator(object):
    ...
    def create_data_dict(self, data_path, data_type, label_path=None):
        ...
        if not os.path.exists(self.config.path.pre_process_data[data_type]):
            ...
            data_dict['pos_ids'], self.pos_dict, self.pos_index = sf.get_pos_feature(self.pos_dict, self.pos_index)
            data_dict['dep_matrix'], data_dict['dep_label_matrix'] = sf.get_dep_feature()
        ...
        if self.config.model_mode == "bert":
            ...
            data_dict['token_char'] = shared_utils.get_stanford_char_bert_char(
                data_dict['standard_token'], data_dict['bert_token'], mapping_col
            )
