...
def pair_stage_model_main(config, pair_representation, make_pair_label, pair_eval, polarity_col,
    ...
    if config.model_mode == "bert":
        ...
        feature_dim = [4 * (5 + config.hidden_size * 2), 4 * 5, 4 * config.hidden_size * 2]
    ...
    if model_parameters['first_stage'] == "polarity":
        # define polarity data loader.
        dev_polarity_loader = data_loader_utils.get_loader([dev_pair_representation], 1)
        ...
        pair_stage_model_test(
            predict_polarity_model, config, dev_polarity_loader, dev_pair_eval,
            test_polarity_parameters, mode="polarity", polarity=True, initialize=(False, False)
        )
        ...
        for epoch in range(50):
            ...
            pair_stage_model_test(
                ...
                dev_pair_parameters, mode="pair", polarity=True, initialize=(False, True)
            )
    ...
    if model_parameters['first_stage'] == "pair":
        pair_stage_model_test(
        ...
        test_polarity_loader = data_loader_utils.get_loader([test_pair_representation], 1)
        pair_stage_model_test(
            predict_polarity_model, config, test_polarity_loader, test_pair_eval,
            test_polarity_parameters, mode="polarity", polarity=True, initialize=(False, False)
        )
        ...
        pair_stage_model_test(
            predict_pair_model, config, test_pair_loader, test_pair_eval,
            test_pair_parameters, mode="pair", polarity=True, initialize=(True, True)
        )
...
def cpc_stage_model_main(config, pair_representation, make_pair_label, pair_eval, polarity_col,
                         model_parameters, optimizer_parameters, model_name, feature_type):
    """
