...
from data_utils import check_utils
...
def commonly_used_classification(model_type, train_feature, train_label, test_feature, test_label):
    model_list = {"svm", "xgboost", "nb", "logistic"}
    assert model_type in model_list, "error model name."
    ...
    if model_type == "logistic":
        ...
        for depth in {2, 3, 4, 5}:
            model = XGBClassifier(max_depth=depth, learning_rate=0.01)
...
def train_commonly_used_classification_model(model_type, train_feature, train_label, test_feature, test_label):
    model_list = {"svm", "xgboost", "nb", "logistic"}
    assert model_type in model_list, "error model name."
    ...
    hyper_param, max_acc, optimize_model = 0.1, -1, None
    while hyper_param < 2:
        classifier = copy.deepcopy(SVC(C=hyper_param))
        classifier.fit(train_feature, train_label)

        y_hat = classifier.predict(test_feature)
        accuracy = accuracy_score(test_label, y_hat)

        print("accuracy is: {:.2f}%".format(accuracy * 100))

        if max_acc < accuracy:
            optimize_model = classifier
            max_acc = accuracy
